...
function ControlPanelMicroserviceCron() {
    ...
    self.init = function() {
        ...
        $("#form_cp_microservice_cron_create").on("submit", "", function(event) {
    ...
    self.changeView = function() {
        if (helper.checkWidthType() === "mobile") {
            ...
            if (selectId >= 0) {
                ...
                var id = $("#cp_microservice_cron_select_result_desktop").find(".checkbox_column input[type='checkbox']").parents("tr").find(".id_column");
            ...
            if (selectSended === true) {
                selectId = $.trim($("#cp_microservice_cron_select_result_desktop").find(".checkbox_column input[type='checkbox']:checked").parents("tr").find(".id_column").text());

                selectSended = false;
            }

            if (selectId > 0)
                $("#cp_microservice_cron_select_mobile").find("select option[value='" + selectId + "']").prop("selected", true);
        }
    };
    ...
    function selectDesktop() {
        ...
        $(document).on("click", "#cp_microservice_cron_select_result_desktop .refresh", function() {
            ajax.send(
                true,
        ...
        $(document).on("click", "#cp_microservice_cron_select_result_desktop .delete_all", function() {
            popupEasy.create(
                window.text.index_5,
                ...
                function() {
                    ajax.send(
                        ...
                        function(xhr) {
                            ...
                            $.each($("#cp_microservice_cron_select_result_desktop").find("table .id_column"), function(key, value) {
        ...
        $(document).on("click", "#cp_microservice_cron_select_result_desktop .cp_microservice_cron_delete", function() {
            var id = $.trim($(this).parents("tr").find(".id_column").text());
            
            deleteElement(id);
        });
        
        $(document).on("click", "#cp_microservice_cron_select_button_desktop", function(event) {
    ...
    function selectMobile() {
        $(document).on("submit", "#form_cp_microservice_cron_select_mobile", function(event) {
            event.preventDefault();
        ...
        $(document).on("change", "#form_microservice_cron_select_id", function() {
            $("#cp_microservice_cron_select_result").html("");
        });
    }
    ...
    function profile(xhr, tag) {
        ...
        if ($.isEmptyObject(xhr.response) === false && xhr.response.render !== undefined) {
            ...
            $("#form_microservice_cron_level").on("keyup", "", function() {
                $(this).val($(this).val().toUpperCase());
            });

            $("#form_cp_microservice_cron_profile").on("submit", "", function(event) {
                event.preventDefault();
            ...
            $("#cp_microservice_cron_delete").on("click", "", function() {
               deleteElement(null);
            });
        }
    }
